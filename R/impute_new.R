...
#' @export
impute_new <- function(path, newdata, m = NULL) {
  ...
  pre.obj <- preprocess(newdata)

  torch.data <- torch_dataset(newdata)
  ...
  for (i in seq_len(m)) {
    ...
    if(is.list(output.data)){
      imp.data <- postprocess(output.data = output.data$reconstrx, pre.obj = pre.obj)
    }else{
