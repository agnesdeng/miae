...
#' @export
tune_dae_dropout<-function(data, m=5, epochs = 10, batch.size = 50,
  ...
  for(i in seq_len(n.models)){
      ...
      model <-dae(n.features = n.features, latent.dim = latent.dim, input.dropout = model.params$input.dropout[i], latent.dropout = latent.dropout, hidden.dropout = model.params$hidden.dropout[i], encoder.structure = encoder.structure, decoder.structure = encoder.structure)
