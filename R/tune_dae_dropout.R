...
#' @export
tune_dae_dropout<-function(data, dropout.grid = list(input.dropout=c(0.2,0.5,0.7),hidden.dropout=c(0.3, 0.6, 0.8)),
  ...
  train.ds <- torch_dataset_idx(data,train.idx, scaler = scaler)
  valid.ds <- torch_dataset_idx(data,valid.idx, scaler = scaler)
  ...
  for(i in seq_len(n.models)){
      ...
      model <-dae(n.features = n.features, input.dropout = model.params$input.dropout[i],  hidden.dropout = model.params$hidden.dropout[i], encoder.structure = encoder.structure, decoder.structure = encoder.structure, act = act)
