...
#' @param optim The name of the optimizer. Options are : "adam" (default), and "sgd".
...
#' @param save.model Whether or not to save the imputation models for different combinations of dropouts. Default: FALSE.
...
#' @export
tune_vae_dropout<-function(data, m=5, epochs = 10, batch.size = 50,
  ...
  for(i in seq_len(n.models)){
    ...
    model <-vae(n.features = n.features, latent.dim = latent.dim, input.dropout = model.params$input.dropout[i], latent.dropout = latent.dropout, hidden.dropout = model.params$hidden.dropout[i], encoder.structure = encoder.structure, decoder.structure = encoder.structure)
