...
midae <- function(data, m = 5, pmm.type = "auto", pmm.k = 5, pmm.link = "prob", pmm.save.vars = NULL,
  ...
  for (epoch in seq_len(epochs)) {
    ...
    if (verbose & (epoch == 1 | epoch %% print.every.n == 0)) {
      if (subsample == 1) {
        cat(sprintf("Loss at epoch %d: %1f\n", epoch, train.loss / length(train.dl)))
      } else if (subsample < 1) {
        ...
        if (is.null(pmm.type)) {
          ...
          if (pmm.type == 0 | pmm.type == 2) {
            if (var %in% pre.obj$num) {
              ...
              data[[var]][na.loc[, var]] <- pmm(yhatobs = yhatobs, yhatmis = yhatmis, yobs = obs.y, k = pmm.k)
              ...
              level.idx <- pmm.multiclass(yhatobs = yhatobs, yhatmis = yhatmis, yobs = obs.y, k = pmm.k)
  ...
  if (isFALSE(save.model)) {
    ...
    print(paste("Imputation model is saved in ", path))
